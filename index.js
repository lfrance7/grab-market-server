...
app.get('/banners', (req, res) => {
    models.Banner.findAll({
        ...
        console.error(error);
        res.status(500).send('banner 관련 에러가 발생하였습니다');
    });
});
...
app.post("/products", (req, res) => {
    ...
    if (!name || !price || !seller || !description || !imageUrl) {
        res.status(400).send("모든 필드를 입력 해주세요");
    }
...
app.post('/purchase/:id', (req, res) => {
    ...
    models.Product.update(
        ...
        .catch((error) => {
            console.error(error);
            res.status(500).send("결제 관련 에러 발생");
        })
});

app.listen(port, () => {
    console.log("그랩의 쇼핑몰 서버가 가동되고 있습니다.");
    models.sequelize.sync().then(() => {
        console.log("DB 연결 성공 !");
    }).catch((err) => {
        console.error(err);
        console.log('DB 연결 에러 ㅜㅜ');
        process.exit();
    });
});